...
## Output Directory (`{{OUTPUT_DIR}}`) — Configurable

All research outputs go to a single configurable directory referenced throughout this skill as `{{OUTPUT_DIR}}`.

**Resolution order (highest priority first):**

1. **Inline parameter** — pass an explicit directory at invocation, e.g. `{{OUTPUT_DIR}}=/some/path`
2. **Environment variable** — set `LITERATURE_SEARCH_OUTPUT_DIR` before running
3. **Default** — `apps/pae-wizard/outputs/research/`

> **Workspace default:** in this repo the default is `apps/pae-wizard/outputs/research/`, which satisfies the workspace rule that all outputs go under `apps/pae-wizard/outputs/`. Override only when porting the skill to another project.

Whenever you see `{{OUTPUT_DIR}}` in this skill or its resources, substitute the resolved directory (no trailing slash required in inline usage; sub-paths are appended with `/`).
...
Save plan to: `{{OUTPUT_DIR}}/search-plan.md`

### Autonomous Mode

When running without interactive TTY (subagent, background worker):

1. Parse research question from existing `{{OUTPUT_DIR}}/search-plan.md`
...
| Source | Type | MCP Tool | Finds |
|--------|------|----------|-------|
| Papers MCP | Multi-platform | `papers__search_papers` | arXiv, OpenAlex, Semantic Scholar, bioRxiv, medRxiv, PubMed |
| PubMed MCP | Biomedical | `pubmed__pubmed_search_articles` | MeSH, PMC fulltext, citations, related |
| `web_search` | General | — | Papers, reviews, news, blogs |
| `web_fetch` journals | PDFs | — | Direct download (open access) |
| LILACS / SciELO | LatAm | — | Mandatory when E_CONTEXT includes LatAm |

> **See [`resources/papers-mcp-protocol.md`](resources/papers-mcp-protocol.md) for complete Papers MCP + PubMed MCP integration guide.**
...
All files go to `{{OUTPUT_DIR}}/` (default: `apps/pae-wizard/outputs/research/`) within the project:
...
| [`papers-mcp-protocol.md`](resources/papers-mcp-protocol.md) | Papers MCP + PubMed MCP integration, search strategy, cache, trend analysis | v1.5.0 🆕 |
...
The output root is configurable via `{{OUTPUT_DIR}}` (see the "Output Directory" section at the top). This workspace defaults to `apps/pae-wizard/outputs/research/` to comply with the repo rule that all outputs live under `apps/pae-wizard/outputs/`. To reuse this skill in another project, override the default via the `LITERATURE_SEARCH_OUTPUT_DIR` environment variable or by passing an inline `{{OUTPUT_DIR}}=...` parameter at invocation — no edits to the skill files are required.

---

**Version:** 1.5.0 | **Updated:** 2026-06-02 | **Changes:** +Papers MCP protocol (papers-mcp-protocol.md), updated source table to include MCP tools as primary search method | **Changes:** +Phase 5 Synthesis & Verification (synthesis-protocol.md), +citation formatting guide APA/Vancouver (citation-format.md), +claim traceability matrix, +conclusion direction audit, +context alignment check, +literature matrix, +writing protocol with absolute claim rules
