...
Use only to create, restructure, or audit technical-presentation scripts, speaker notes, and slide outlines. Establish audience, intended outcome, timing, source evidence, and the conclusion or decision before drafting. Ask one high-impact question only when a missing fact prevents a faithful result; otherwise state bounded assumptions.
...
- Keep one defensible thesis; distinguish observation, inference, hypothesis, recommendation, and projection.
...
| Audience must assess research | research pattern |
| Audience must decide | technical-decision pattern |
| Audience must examine a failure | incident pattern |
...
1. Write a complete thesis with its evidence and limits.
2. Order the reasoning: current model or problem, question, discriminating evidence, alternatives, conclusion, uncertainty, and implication.
