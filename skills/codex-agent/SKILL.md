...
- Fijar siempre `-s` explícito; nunca heredar el de config (lectura: `-s read-only`; escritura con aviso: `-s workspace-write`). El approval lo gobierna la config de Codex — esta versión de `codex exec` no expone flag `-a`/`--approval`, solo `-s`/`--sandbox`.
...
timeout 60 codex exec -C <DIR> -s read-only -m "$MODEL" \
  -o "$OUT" "<prompt del worker>" </dev/null
```
...
- One-task-per-launch: una tarea acotada por corrida; post-ejecución revisar `git -C <DIR> diff --stat` y `git status --short` antes de declarar done.
...
OUT=$(mktemp /tmp/codex-out.XXXXXX)
timeout 60 codex exec -C "$PWD" -s read-only -m "$MODEL" -o "$OUT" "lista los archivos del directorio actual sin modificar nada" </dev/null
cat "$OUT"
```
...
- Minis (`gpt-5.4-mini` y familia): agregar `-c model_reasoning_effort="low"` o usar un modelo apto (el `effort max` de config es incompatible y falla con `400 unsupported_value`).
