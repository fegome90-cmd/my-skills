...
<!-- LOCAL ADDITION: wiki reference pointer (progressive disclosure, opt-in) -->

## Deeper reference (optional)

A persistent LLM Wiki about Herdr (install, socket API, configuration,
keybindings, agent automation, plugin lists, tmux comparison) lives in the
shared vault at `~/.llm-wiki/wiki/`. It is available for clarification when
this skill's operational surface is not enough.

Load `resources/wiki-reference.md` ONLY when you need to clarify a concept,
flag, or behavior not covered above. Do not read it by default — it is an
opt-in index, not required context.
